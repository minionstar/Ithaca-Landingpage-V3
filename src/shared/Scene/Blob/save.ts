...
export const liquedPointsMaterial = ({ material }: any) => {
    ...
    material.onBeforeCompile = shader => {
        ...
        shader.uniforms.baseColor = { value: { r: .0, g: .2, b: .18 } }
        ...
        shader.vertexShader = `
            ...
            vec4 getLiquedPosition() {
                ...
                float uDisplacementFrequency = .7;
                float uDisplacementStrength = .4;
        ...
        shader.fragmentShader = shader.fragmentShader.replace('#include <dithering_fragment>', `
            ...
            float transparencySectionFour = smoothstep(-15., 1., vPosition.z) * (1. - smoothstep(0., 8., vPosition.z));
            ...
            float clampShiftZ = (smoothstep(-5., 10., vPosition.z) + smoothstep(0., 6., vPosition.z));
            ...
            float shiftColor = smoothstep(0., 2., perling * 20.);
            float shiftCovered = (perling + .1);

            // White Color
            vec3 whiteColor = vec3(.1) + (shiftColor+shiftCovered);

            // Green Color
            vec3 greenColor = vec3(.0, .0+shiftColor*2.8, 0.);
